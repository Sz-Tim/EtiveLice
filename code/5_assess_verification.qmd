...
sim_finished <- dir(sim_dir, "posterior_summary", recursive=T) |> str_sub(1, 6) |> unique()
out_df <- sim_finished |>
  map_dfr(
    ~full_join(
      data.table::fread(glue("{sim_dir}/{.x}/posterior_summary_LOOSE.csv")) |> as_tibble(),
      data.table::fread(glue("{sim_dir}/{.x}/params_LOOSE.csv")) |> as_tibble(), 
      by=join_by(name)
    ) |>
      mutate(sim=paste0("dat_", str_sub(.x, -2, -1))) |>
...
pAttach_post <- sim_finished |>
  map_dfr(
      ~bind_rows(
        read_csv(glue("{sim_dir}/{.x}/posterior_LOOSE.csv"), show_col_types=F) |>
          filter(grepl("attach_beta", name)) |>
          select(-.iteration, -.chain) |>
          mutate(fit="Model"),
        read_csv(glue("{sim_dir}/{.x}/params_LOOSE.csv"), show_col_types=F) |>
          filter(grepl("attach_beta", name)) |>
