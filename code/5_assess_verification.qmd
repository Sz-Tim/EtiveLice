...
out_df <- sim_finished |>
  map_dfr(
    ~full_join(
      read_csv(glue("{sim_dir}/{.x}/posterior_summary.csv"), show_col_types=F),
...
attach_df <- expand_grid(
  ...
  Sal=seq_quantiles(c(attach_mx[,,2]), 0.01, 0.99, length.out=2),
  Temp=seq_quantiles(c(attach_mx[,,3]), 0.005, 0.995, length.out=3),
...
attach_df2 <- expand_grid(
  ...
  Temp=seq_quantiles(c(attach_mx[,,3]), 0.005, 0.995, length.out=3),
  UV=seq_quantiles(c(attach_mx[,,4]), 0, 0.995,  length.out=2)
) |>
  mutate(UV_sq=UV^2) 
...
p <- pAttach_post |>
  filter(.draw %in% c(1, sample.int(n_distinct(pAttach_post$.draw), 50))) |>
  filter(sim %in% sample(unique(pAttach_post$sim), 7)) |>
  ggplot(aes(UV_raw, pAttach, alpha=fit, linewidth=fit,
             group=paste(RW, salinity, temperature, .draw, fit), colour=temperature)) + 
  geom_line() +
  scale_colour_viridis_c(option="turbo", end=0.9) +
...
p <- pAttach_post2 |>
  filter(.draw %in% c(1, sample.int(n_distinct(pAttach_post$.draw), 50))) |>
  filter(sim %in% sample(unique(pAttach_post2$sim), 7)) |>
...
```{r}
selected_sims <- sample(unique(pSurv_post$sim), 7)
