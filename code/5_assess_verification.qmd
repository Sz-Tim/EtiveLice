...
sim_finished <- dir(sim_dir, "posterior_summary_LOOSE", recursive=T) |> str_sub(1, 6) |> unique()
out_df <- sim_finished |>
  map_dfr(
    ~full_join(
      read_csv(glue("{sim_dir}/{.x}/posterior_summary_LOOSE.csv"), show_col_types=F),
...
ggsave(glue("{proj_dir}/admin/project_meetings/figs_temp/NEW_pop_freeDet_performance_LOOSE.png"), p,
...
p <- full_sum_df |>
  ...
  labs(x="True value",
       ...
       title="Verification across simulated datasets")
ggsave(glue("{proj_dir}/admin/project_meetings/figs_temp/NEW_pop_freeDet_performance_all_LOOSE.png"), p,
