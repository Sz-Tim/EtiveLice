...
pen_df <- st_read("data/pen_sites.gpkg") |>
  ...
  group_by(sepaSite) |>
  mutate(nPens=n(),
         pen=paste0(sepaSite, "_", str_pad(row_number(), 2, "left", "0"))) |>
  ungroup() |>
  arrange(pen)
...
# Download data
download_lice_counts("2023-01-01", "2025-12-31", "data/ms_sea_lice_2021-2025.csv")
download_ms_site_details("data/ms_site_details.csv")
download_sepa_licenses("data/se_permit_conditions.csv")
download_fish_biomass("2023-01-01", "2026-03-31", "data/se_monthly_reports.csv")
...
lice_compiled <- read_csv("data/ms_sea_lice_2021-2025.csv") |>
...
biomass_interp <- left_join(
  expand_grid(sepaSite=unique(biomass_df$sepaSite),
              date=seq(ymd("2023-01-31"), ymd("2026-01-31"), by=1)) |>
...
out.df <- combo.df |>
  ...
  mutate(total_AF=weeklyAverageAf * actualBiomassOnSiteTonnes * 280) |>
  arrange(sepaSite, date) |>
  group_by(sepaSite) |>
...
pen_df |>
  ...
  select(pen, easting, northing) |>
  write_csv("data/pen_sites_linnhe_2025.csv")
...
# Combining WeStCOMS2/3 with etive28
library(sf)
...
wce_raw <- st_read("temp/westcoms3_etive28_union_unresolvedOverlap.gpkg")
wce <- wce_raw |>
  mutate(ii=if_else(is.na(i_2), i, i_2),
         dd=if_else(is.na(depth_2), depth, depth_2),
         tn1=if_else(is.na(trinode_1_2), trinode_1, trinode_1_2),
  ...
  mutate(area=st_area(.),
         vol=as.numeric(area)*dd) |>
  rename(depth=dd, trinode_1=tn1, trinode_2=tn2, trinode_3=tn3)
st_write(wce, "../03_packages/WeStCOMS/data/WeStCOMS3_etive28_mesh.gpkg")
