...
for(sim in 6:7) {
  ...
  out_pt1 <- stan(file="code/stan/IEM_pt1_ensAttach.stan", data=stan_dat$dat,
                     chains=3, cores=3, iter=2000, refresh=100,
                     init=0,
                     pars=c("attach_beta", "nb_prec", "IP_bg", "IP_bg_m3", "ensWts_p"))
  saveRDS(out_pt1, glue("{dat_dir}/out_pt1_stanfit.rds"))

  out_pt1 <- readRDS(glue("{dat_dir}/out_pt1_stanfit.rds"))
  out_pt1_df <- as.data.frame(out_pt1) |>
    select(-lp__) |>
  ...
  out_pt2_pop <- stan(file="code/stan/IEM_pt2_onFish_pop.stan",
                     data=stan_dat$dat, chains=3, cores=3, iter=1000, refresh=1,
                     # control=list(max_treedepth=20),
                     init=0, seed=10,
                     pars=c("surv_beta", "pMoltF_beta", "pMoltM_beta", "nb_prec"))
  saveRDS(out_pt2_pop, glue("{dat_dir}/out_pt2_pop_stanfit.rds"))


  out_pt2_pop_df <- as.data.frame(out_pt2_pop) |>
    select(-lp__) |>
    mutate(iter=row_number()) |>
    pivot_longer(-iter)
  write_csv(out_pt2_pop_df, glue("{dat_dir}posterior_pt2_pop.csv"))
  out_pt2_pop_sum <- out_pt2_pop_df |>
    group_by(name) |>
    sevcheck::get_intervals(value, type="qi")
  write_csv(out_pt2_pop_sum, glue("{dat_dir}posterior_summary_pt2_pop.csv"))

  dat_pt2_pop_df <- tibble(
  ...
  surv_ls <- map(list(out_pt2_pop_df, out_pt2_pop_sum, dat_pt2_pop_df),
                 ~.x |> filter(grepl("surv_beta", name)) |>
                   inner_join(param_key, by=join_by(name)))
  p_surv <- post_summary_plot(surv_ls, ncol=4, scales="free") +
    geom_vline(xintercept=0, linetype=3)

  pMoltInt_ls <- map(list(out_pt2_pop_df, out_pt2_pop_sum, dat_pt2_pop_df),
                     ~.x |> filter(grepl("pMolt.*_beta\\[1", name)) |>
                       inner_join(param_key, by=join_by(name)))
  p_pMoltInt <- post_summary_plot(pMoltInt_ls, ncol=5, scales="free_y") +
    geom_vline(xintercept=0, linetype=3)

  pMoltTemp_ls <- map(list(out_pt2_pop_df, out_pt2_pop_sum, dat_pt2_pop_df),
  ...
  out_pt3 <- stan(file="code/stan/IEM_pt3_detection.stan",
                        data=stan_dat$dat, chains=3, cores=3, iter=3000, refresh=100,
                        init=0)
  saveRDS(out_pt3, glue("{dat_dir}/out_pt3_stanfit.rds"))

  out_pt3 <- readRDS(glue("{dat_dir}/out_pt3_stanfit.rds"))
  out_pt3_df <- as.data.frame(out_pt3) |>
    select(-lp__) |>
    mutate(iter=row_number()) |>
    pivot_longer(-iter)
  ...
  out_full <- stan(file="code/stan/IEM_full_FEMALE.stan",
                   data=stan_dat$dat, chains=1, cores=3, iter=1, refresh=1,
                   # control=list(max_treedepth=20),
                   init=0, seed=101,
  ...
  # )
  #
  # ggplot(out_full_df |> filter(name != "thresh_GDD[3,2]")) +
  #   # stat_pointinterval(.width=c(0.5, 0.8, 0.95)) +
  #   geom_density(aes(x=value)) +
  #   geom_pointrange(data=out_full_sum |> filter(name != "thresh_GDD[3,2]"),
  #                   aes(x=mn, xmin=L25, xmax=L75, y=0), linewidth=1.25) +
  #   geom_linerange(data=out_full_sum |> filter(name != "thresh_GDD[3,2]"),
  #                  aes(xmin=L10, xmax=L90, y=0), linewidth=0.8) +
  #   geom_linerange(data=out_full_sum |> filter(name != "thresh_GDD[3,2]"),
  #                  aes(xmin=L025, xmax=L975, y=0), linewidth=0.5) +
  #   ylim(0, NA) +
  #   geom_point(data=dat_full_df |> filter(name != "thresh_GDD[3,2]"),
