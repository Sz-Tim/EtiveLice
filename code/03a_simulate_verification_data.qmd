...
This document generates data for verification of the integrated population model (IPM). The IJPM is a multilevel Bayesian model that combines copepodid infestation pressure (IP) from a set of particle tracking simulation variants, calculates ensemble IP as a weighted average across the simulation variants, estimates attachment rates, and models the on-fish progression of daily lice cohorts through time. Observed counts of each stage of lice each week are used as the response variable, with all model parameters estimated jointly. The IPM will be verified using data that is simulated using identical processes with known, realistic parameters to assess the ability of the JPM to accurately and precisely recover parameters.

This script can be modified to use final `biotracker` simulations, best available farm data and the final parameter sets. For the moment, it uses daily averages from `etive28` at each farm with a sample of `biotracker` simulations, running from 2023-01-01 to 2025-12-31.
...
MOWI_farms <- read_csv(glue("{proj_dir}/data/aquaculture/mowi_cleaned.csv")) |>
  count(sepaSite) |> select(sepaSite) |> unlist() |> unname()
...
inputs_dir <- glue("{proj_dir}/data/biotracker/2022_2025")
```
...
`farm_i`: Farm metadata. Location and volume = $\sum{Area_i \times \min(depth_i, 30)}$ for elements $i$ clipped to the 30m radius around each farm. The 20m depth limit and 30m radius corresponds with the connectivity calculations as parameterized in biotracker (*2_run_biotracker.R*).

```{r}
#| label: "pen_df"
pen_df <- read_csv(glue("{proj_dir}/data/pen_sites_widerLinnhe_2022-2025.csv"), 
                   show_col_types=F) |>
  mutate(sepaSite=str_split_fixed(pen, "_", 2)[,1]) |>
  filter(sepaSite %in% MOWI_farms) |>
  group_by(sepaSite) |>
  mutate(nPens=n()) |>
  ungroup()
glimpse(pen_df)
farm_order <- sort(unique(pen_df$sepaSite))
```

```{r}
#| label: "farm_i"
farm_i <- read_csv(glue("{proj_dir}/data/farm_sites_widerLinnhe_2022-2025.csv"),
                   show_col_types=F) |>
...
ggplot() +
  ...
  geom_sf_text(data=farm_sf, aes(label=sepaSite), hjust=0, nudge_x=250, size=2.5) +
  scale_x_continuous(limits=c(144000, 218000), oob=scales::oob_keep) + 
  scale_y_continuous(limits=c(725000, 780000), oob=scales::oob_keep) +
  theme(axis.title=element_blank(),
        axis.text=element_blank())
```

Note that Mowi has integrated SAR1 into APT1. That doesn't seem to be reflected on the governmental aquaculture website.

`farm_dat`: Daily interpolation of mean adult female lice per fish, reported fish biomass, estimated number of fish assuming 240 fish per tonne, estimated total adult female lice assuming 240 fish per tonne,, receiving area ratio, logit-transformed receiving area ratio, and whether lice were sampled on a given day. Receiving area ratio is calculated following @kragesteen2023 as $R(W_i) = \sqrt{\frac{W_i}{DV_r}}$ (recommended Method 3), with biomass $W$ = `actualBiomassOnSiteTonnes * 1000` ($kg$), stocking density $D = 20 kg \cdot m^{-3}$, and receiving area $V_r$ = `vol` ($m^3$), which corresponds with the influx volume from the biotracer simulations. Thus, $R(W_i)$ = `RW` is the proportion of copepodids that successfully attach.
...
farm_dat <- read_csv(glue("{inputs_dir}/farm_dat.csv"), show_col_types=F) |>
  ...
  mutate(nFishSampled=if_else(sampled, sample(10:25, n(), replace=T), 0))
glimpse(farm_dat)
```
...
p_ls <- list(farm_dat_gg + geom_line(aes(y=nFish_est)),
             ...
             farm_dat_gg + geom_line(aes(y=BSA)))
cowplot::plot_grid(plotlist=p_ls, ncol=1, align="hv")

if(save_figs) {
  cowplot::plot_grid(farm_dat_gg +
                       geom_line(aes(y=nFish_est)) +
                       ylab("Number of fish"),
                     farm_dat_gg +
                       geom_line(aes(y=weeklyAverageAf * (nFish_est > 0))) +
...
From `etive28`, I've calculated daily averages across the top 30m at each farm location to summarise the environmental conditions.
...
farm_env <- read_csv(glue("{inputs_dir}/farm_env_hourly.csv"), show_col_types=F) |>
  ...
  inner_join(
    farm_dat |> 
      select(sepaSite, date, nFish_est, RW, RW_logit, BSA, sampled, nFishSampled), 
...
farm_env_daily |>
  ...
  scale_x_date(date_breaks="6 months", date_labels="%b", date_minor_breaks="2 month") +
  labs(y="Daily mean, 0-30m (Etive28, WeStCOMS2)") +
...
Copepodid infestation pressure was taken from 10 `biotracker` simulations which randomly sampled from a plausible parameter space. The number of copepodids in the top 30m within a 30m radius of each pen was totaled each day. The number of simulations will increase as testing progresses. The parameters that varied were:
...
read_csv(glue("E:/EtiveLice/out/jpm/sim_i.csv")) |>
...
ggplot(influx_df, aes(time, influx_m3, colour=sepaSite, group=pen)) +
  ...
  scico::scale_colour_scico_d(palette="devon", end=0.8, direction=1, guide="none") +
  scale_x_date(date_breaks="6 months", date_labels="%b", date_minor_breaks="2 month") +
  scale_y_continuous("Mean daily copepodids / m3 in top 30m") +
...
attach_mn <- c(-3.5, 1, 0.2, -0.2, -0.1, 0.2, -0.1) # Int, BSA, Sal, Temp, Temp^2, uv, uv^2
attach_sigma <- 0.7
attach_df <- expand_grid(
  salinity_z=seq_range(farm_env$salinity_z, length.out=100),
  uv_z=seq_range(farm_env$uv_z, length.out=100),
  BSA_z=seq_range(filter(farm_env, BSA_z>0)$BSA_z, length.out=100),
  temp_z=seq_range(farm_env$temperature_z, length.out=100)
) |>
  slice_sample(n=50) |>
  mutate(salinity=salinity_z*farm_env_avg$salinity[2] + farm_env_avg$salinity[1],
  ...
  mutate(eta=attach_mn[1] + 
           ...
           attach_mn[4] * temp_z +
           attach_mn[5] * temp_z^2 +
           attach_mn[6] * uv_z +
           attach_mn[7] * uv_z^2,
         obs=eta + rnorm(n(), 0, attach_sigma))
ggplot(attach_df, aes(uv, plogis(eta), colour=BSA_z)) + geom_point() + ylim(0, 1)
ggplot(attach_df, aes(temp, plogis(eta), colour=BSA_z)) + geom_point() + ylim(0, 1)
ggplot(attach_df, aes(BSA_z, plogis(eta), colour=uv_z)) + geom_point() + ylim(0, 1) + 
  scale_colour_viridis_c(option="turbo")
ggplot(attach_df, aes(BSA_z, plogis(eta), colour=temp)) + geom_point() + ylim(0, 1) + 
  scale_colour_viridis_c(option="turbo")
plot_ly(attach_df, x=~uv_z, y=~BSA_z, z=~salinity, color=~plogis(eta), type="scatter3d")
plot_ly(attach_df, x=~uv_z, y=~BSA_z, z=~temp_z, color=~plogis(eta), type="scatter3d")
summary(plogis(attach_df$eta))
hist(plogis(attach_df$eta), xlim=c(0,1))


attach_out <- brm(obs ~ BSA_z + salinity_z + temp_z + I(temp_z^2) + uv_z + I(uv_z^2),
...
surv_mn <- cbind(qlogis(c(0.95, 0.95, 0.98, 0.98, 0.99)),
                 c(0.2, 0.2, 0.18, 0.18, 0.18))
surv_sigma <- 0.7
surv_df <- expand_grid(
  ...
  mutate(eta=surv_mn[stage,1] + surv_mn[stage,2]*salinity,
         ...
         sal5=as.numeric(stage==5)*salinity)
surv_out <- brm(obs ~ 0 + int1 + int2 + int3 + int4 + int5 + 
...
trt_meth_ii <- read_csv(glue("{proj_dir}/data/aquaculture/mowi_trt_cleaned.csv")) |>
  count(MethodNum, TypeNum, Method, Type) |>
  arrange(TypeNum)
info <- list(nDays=n_distinct(influx_df$date),
             ...
             trt_proportions=trt_meth_ii$n/sum(trt_meth_ii$n),
             IP_penVolume=(read_csv(glue("{inputs_dir}/farm_influx_vols.csv"),
                                    show_col_types=F) |>
                             select(pen, vol) |>
                             mutate(sepaSite=str_split_fixed(pen, "_", 2)[,1]) |>
...
params <- list(
  ...
  # Coefficients: logit-scale yday harmonic regression (Int, cos, sin)
  ensWts_harm=matrix(c(log(rlnorm(info$nSims, sdlog=1)), rnorm(info$nSims*2)), 
                     ncol=info$nSims, nrow=3, byrow=T),
  # Pr(copepodid attachment): RW_logit, salinity, temp, uv, uv^2
  attach_beta=glue("{proj_dir}/data/sim/inputs/posterior_prAttach.csv") |>
    read_csv() |> sample_n(1) |> 
    select(b_Intercept, b_BSA_z, b_salinity_z, b_temp_z, b_uv_z, b_Itemp_zE2, b_Iuv_zE2) |> unlist(),
  # On-fish survival: chalimus, pre-adult, adult, gravid
  #   logit(p) = f(salinity); intercept, slope by stage
  surv_beta=glue("{proj_dir}/data/sim/inputs/posterior_prSurv.csv") |>
  ...
  surv_int_farm_sd=abs(brms::rstudent_t(info$nStages, 3, 0, 0.75)),
  # Cumulative GDD post-attachment needed to transition to next stage
  thresh_GDD=cbind( 
    runif(5, min=c(62.5, 125, 225, 325, 525), max=c(87.5, 175, 275, 375, 575)), # F: cIII/cIV/cV/Ad/Gr
    runif(5, min=c(52.5, 105, 197.5, 290, 5e3), max=c(77.5, 155, 247.5, 340, 5e3)) # M: cIII/cIV/cV/Ad/NA
  )[1:(info$nStages-1),],
  ...
  trt_thresh=sample(c(0.5, 1), 1),
  trt_efficacy_method=rbeta(info$nTrtMethods, 4, 2)
)
params$trt_efficacy <- boot::inv.logit(
  boot::logit(params$trt_efficacy_method[info$trt_meth_ii]) + rnorm(info$nTrtTypes, 0, 1)
)
# Randomly set salinity, temp, and uv effects to 0 for attachment
params$attach_beta[3:7] <- params$attach_beta[3:7] * rbinom(3, 1, 0.75)[c(1,2,2,3,3)]
# Randomly set salinity effects to 0 for survival
params$surv_beta[2,] <- params$surv_beta[2,] * rbinom(info$nStages, 1, 0.75)
...
# Mean days per stage: for specifying priors; assume Ch1=Ch2, PA1=PA2: days/2
...
p_prAttach <- expand_grid(
  ...
  BSA_z=quantile(farm_env_daily$BSA_z, probs=c(0.5, 0.95)),
  ...
  mutate(uv_sq=uv_z^2,
         ...
         BSA=BSA_z*farm_env_avg$BSA[2],
         BSA_lab=factor(BSA, labels=c("BSA: Median", "BSA: 95th %ile"))) |>
  mutate(pr_attach=plogis(params$attach_beta[1] +
                            params$attach_beta[2] * BSA_z +
                            params$attach_beta[3] * salinity_z +
                            params$attach_beta[4] * temp_z +
                            params$attach_beta[5] * uv_z +
                            params$attach_beta[6] * temp_sq +
                            params$attach_beta[7] * uv_sq)) |>
  ggplot(aes(salinity, uv)) + 
  geom_raster(aes(fill=pr_attach)) + 
  geom_contour(aes(z=pr_attach), breaks=seq(0, 1, by=0.1), colour="white") +
  scale_fill_viridis_c("P(attach)", option="turbo", 
                       limits=c(0, 1), breaks=seq(0, 1, by=0.1),
                       labels=c("0", rep("", 4), "0.5", rep("", 4), "1")) +
  facet_grid(BSA_lab~temp_lab) +
...
p_prAttachIP <- expand_grid(
  IP=seq_quantiles(influx_df$influx_m3, 0, 0.9995, by=0.01)*max(info$IP_penVolume),
  maxPrAttach=seq(0, 1, by=0.01)
  ) |>
  mutate(pr_attach=(maxPrAttach*max(params$IP_halfSat))/(IP + max(params$IP_halfSat))) |>
  ggplot(aes(IP/max(info$IP_penVolume), maxPrAttach)) + 
  geom_raster(aes(fill=pr_attach)) +
  geom_contour(aes(z=pr_attach), breaks=seq(0, 1, by=0.1), colour="white") +
...
p_ensWts <- as_tibble(make_ydayh_mx() %*% params$ensWts_harm) |>
  set_names(unique(influx_df$sim)) |>
  mutate(ydayh=row_number()) |>
  pivot_longer(-ydayh, names_to="sim", values_to="logit_p") |>
  group_by(ydayh) |>
  mutate(p=make_compositional(logit_p)) |>
  ungroup() |>
  mutate(date_std=ymd_h("2020-01-01 00") + dhours(ydayh - 1)) |>
  ggplot(aes(date_std, p, colour=sim)) + 
  geom_line() + 
...
p_prSurv <- expand_grid(
  ...
  scale_colour_manual(values=RColorBrewer::brewer.pal(4, "Paired")[c(1,1,2,2,4)]) +
  scale_fill_manual(values=RColorBrewer::brewer.pal(4, "Paired")[c(1,1,2,2,4)]) +
...
p_daysStage <- tibble(Sex=rep(c("F", "M"), each=info$nStages-1),
                      ...
                      StageGroup=rep(info$stg_grp_ii[1:4], times=2),
                      GDD=c(rbind(params$thresh_GDD))) |>
  slice_tail(by=c(Sex, StageGroup)) |>
  group_by(Sex) |>
  mutate(GDD=GDD-dplyr::lag(GDD, default=0)) |>
  ungroup() |>
  ...
  geom_line() +
  scale_colour_manual("Transition", values=RColorBrewer::brewer.pal(3, "Paired")[c(1,2)]) +
  ...
  theme(legend.position="inside",
        ...
        legend.background=element_blank())
# detection
p_detect <- tibble(Stage=factor(1:info$nStageGroups, 
                                labels=c("Ch", "PA", "Ad")),
...
if(use_farm_avg) {
  pr_surv_df <- expand_grid(stage=stages,
...
#| fig-width: 10
#| fig-height: 10
#| fig-cap: "Daily mean lice per fish for adult females including latent (mu, lines) and observed (y, points)."
#| fig-dpi: 400
#| echo: false
...
ybar_farm <- ybar_df |>
  ...
  summarise(value=mean(value)) |>
  ungroup()
ggplot(mu_df |> filter(stage == "af"), aes(date)) + 
  geom_hline(yintercept=0.5, linetype=2) + 
  geom_line(aes(y=value, group=pen), colour="cadetblue", alpha=0.5, linewidth=0.1) +
  geom_line(data=mu_farm |> filter(stage == "af"), aes(y=value),
            colour="cadetblue", linewidth=0.9) +
  geom_jitter(data=y_df |> filter(stage == "af"), aes(y=value),
              shape=1, size=0.8, width=0.2, height=0) +
  geom_point(data=y_farm |> filter(stage == "af"), aes(y=value), shape=4, size=2) +
  geom_rug(data=out_df |> filter(treat==1), sides="b", colour="black") +
  scale_x_date(date_breaks="6 months", date_labels="%b", date_minor_breaks="2 month") +
  labs(y="Lice per fish") +
