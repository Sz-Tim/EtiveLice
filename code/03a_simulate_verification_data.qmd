...
farm_i <- read_csv(glue("{proj_dir}/data/farm_sites_widerLinnhe_2022-2025.csv"),
  ...
  left_join(pen_df |> group_by(sepaSite) |> summarise(nPens=first(nPens)), 
            by="sepaSite") |>
  left_join(read_csv(glue("{proj_dir}/data/aquaculture/mowi_farms_mapped_areas.csv"))) |>
  arrange(sepaSite)
farm_i
```
...
farm_dat <- read_csv(glue("{inputs_dir}/farm_dat.csv"), show_col_types=F) |>
  right_join(farm_i |> select(sepaSite, firstDay, nPens), 
             by=join_by(sepaSite)) |>
  ...
  mutate(sepaSite=factor(sepaSite, levels=farm_order),
         ...
         RW=sqrt(fishIn*maximumBiomassAllowedTonnes*1e3/(20*nPens*20*pi*30^2)),
...
params <- list(
  ...
  treat_efficacy_method=rbeta(info$nTrtMethods, 4, 2),
  treat_efficacy=rbeta(info$nTrtTypes, 4, 2)
)
params$treat_efficacy <- boot::inv.logit(
  boot::logit(params$treat_efficacy_method[info$trt_meth_ii]) + rnorm(info$nTrtTypes, 0, 1)
)
