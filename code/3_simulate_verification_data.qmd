...
```{r}
pen_df <- read_csv(glue("{inputs_dir}/pen_sites_etive_2023.csv"), show_col_types=F) |>
...
farm_dat <- read_csv(glue("{inputs_dir}/farm_dat.csv"), show_col_types=F) |>
  ...
  # mutate(actualBiomassOnSiteTonnes=pmax(actualBiomassOnSiteTonnes, 200)) |>
  mutate(sepaSite=factor(sepaSite, levels=farm_order),
         nFish_est=actualBiomassOnSiteTonnes*240/nPens,
         total_AF=weeklyAverageAf * nFish_est,
         RW=sqrt(actualBiomassOnSiteTonnes/nPens*1000/(20*20*pi*30^2)),
...
farm_dat_gg <- ggplot(farm_dat, aes(x=date)) + 
  ...
  scale_x_date(date_breaks="2 months", date_labels="%b", date_minor_breaks="1 month") +
  ylim(0, NA) + 
...
Estimation of fish numbers is **terrible**, but will serve for now. I also need to deal with 0's in the model.
:::
...
Copepodid infection pressure was taken from 6 `biotracker` simulations which randomly sampled from a plausible parameter space. The number of copepodids in the top 20m within a 30m radius of each pen was totaled each day. The number of simulations will increase as testing progresses. The parameters that varied were:
...
#| fig-width: 10
#| fig-height: 6
#| fig-cap: "Daily copepodid infection pressure (4th root) by farm."
#| fig-dpi: 400
#| echo: false
...
The simulations vary quite a lot in the infection pressure, with considerable variation among pens as well.
...
if(rbinom(1, 1, 0.5)) {
  params$attach_beta <- c(qlogis(0.4), 0.7, 0, 0, 0)
} else {
  params$attach_beta <- glue("{inputs_dir}/posterior_prAttach.csv") |>
...
if(rbinom(1, 1, 0.5)) {
  params$surv_beta <- rbind(qlogis(c(0.95, 0.98, 0.99, 0.97)),
                            c(0, 0, 0, 0))
} else {
  params$surv_beta <- glue("{inputs_dir}/posterior_prSurv.csv") |>
...
# development
p_GDD <- tibble(Sex=rep(c("F", "M"), each=4),
                Stage=rep(1:4, times=2),
...
#| fig-width: 10
#| fig-height: 10
#| fig-cap: "Ensemble daily copepodid infection pressure. Farms are ordered from uploch (light) to downloch (dark). Thin lines are individual pens with thick lines as farm-level averages."
#| fig-dpi: 400
#| echo: false
#| message: false
...
#| fig-width: 10
#| fig-height: 4
#| fig-cap: "Daily copepodid infection pressure by simulation. Farms are ordered from uploch (light) to downloch (dark). Thin lines are individual pens with thick lines as farm-level averages."
#| fig-dpi: 400
#| echo: false
...
::: callout-note
There are several challenges to appropriately dealing with pens within a farm. Currently, we assume a point location with a 100-200m radius defining the catchment area for copepodids and ignore the existence of separate pens.

-   If pens are treated separately:
    -   Pens could be implemented as functionally separate sites, with all variables, parameters, and lice estimates tracked independently by pen (FFMC84_1, FFMC84_2, etc).
    -   Relocation of fish among pens would need to be accounted for.
    -   Catchment areas would overlap. Allowing double counting would over-estimate infection pressure as the same lice represented by a given particle could infect multiple pens. Options for dealing with this:
        -   Ignore it, assuming a louse has a \~20% chance of infecting pen 1 and \~20% chance of infecting pen 2 and calculate totals independently
        -   Do something complicated where we calculate the proportion of overlapping area and reduce attachment accordingly
        -   Keep catchment areas small enough to be non-overlapping and simulate more particles to compensate
-   If pens are ignored:
    -   A 100-200m catchment radius does not actually capture the footprint of the farm; pens are typically in a 2 x n grid and pens are directionally excluded from the catchment area. Extending the circular radius to include the full pen would also include a large area beyond the footprint in the shorter dimension. Non-circular catchment areas are not currently supported in `biotracker`, but could be implemented by reading in shapefiles or using convex hulls.
    -   Replication is ignored which could help improve the parameterization.
:::
...
mu_df |>
  ...
  scale_x_date(date_breaks="2 months", date_labels="%b", date_minor_breaks="1 month") +
  scale_fill_manual(values=RColorBrewer::brewer.pal(n=6, name="Paired")[-4]) +
