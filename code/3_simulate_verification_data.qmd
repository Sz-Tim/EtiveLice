...
influx_df <- read_csv(glue("{inputs_dir}/influx_hourly.csv"), show_col_types=F) |>
  ...
  complete(time, pen, sim, fill=list(influx=0, influx_m2=0, influx_m3=0)) |>
  mutate(date=date(time),
         day=as.numeric(as.factor(date)),
         hour=hour(time),
         sepaSite=str_split_fixed(pen, "_", 2)[,1],
         sepaSite=factor(sepaSite, levels=farm_order)) |>
  select(time, date, day, hour, sepaSite, pen, sim, influx, influx_m2, influx_m3) |>
...
params$IP_halfSat <- params$IP_halfSat_m3*pi*30^2*20
...
p_prAttachIP <- expand_grid(
  IP=seq(0, 10, by=0.1)*info$IP_penVolume,
  maxPrAttach=seq(0, 0.75, by=0.01)
  ) |>
  mutate(pr_attach=(maxPrAttach*params$IP_halfSat)/(IP + params$IP_halfSat)) |>
  ggplot(aes(IP/info$IP_penVolume, maxPrAttach, fill=pr_attach)) + 
  geom_raster() +
