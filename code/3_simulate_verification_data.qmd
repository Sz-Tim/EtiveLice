...
prod_cycles <- fread(glue("{inputs_dir}/farm_dat.csv")) |> as_tibble() |>
...
farm_dat <- fread(glue("{inputs_dir}/farm_dat.csv")) |> as_tibble() |>
...
```{r}
#| label: "farm_env"
farm_env <- fread(glue("{inputs_dir}/farm_env_hourly.csv")) |> as_tibble() |>
  filter(sepaSite %in% farm_order) |>
  mutate(sepaSite=factor(sepaSite, levels=farm_order),
         date=date(time),
         hour=hour(time)) |>
...
```{r}
#| label: "influx_df"
influx_df <- fread(glue("{inputs_dir}/influx_hourly.csv")) |> as_tibble() |>
  filter(sepaSite %in% farm_order) |>
