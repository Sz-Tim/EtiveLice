...
```{r}
sim_finished <- dir(sim_dir, glue("mu_fitted_metrics_weekly{ifelse(priors, '_PRIORS', '')}.csv"), recursive=T) |> 
  str_sub(1, 6) |> unique()
metric_wk_df <- map_dfr(
  sim_finished,
  ~read_csv(glue("{sim_dir}/{.x}/mu_fitted_metrics_weekly{ifelse(priors, '_PRIORS', '')}.csv"), show_col_types=F) |>
    mutate(sim=paste0("dat_", str_sub(.x, -2, -1)))) 
...
ggsave(glue("{proj_dir}/admin/project_meetings/figs_temp/performance_metrics_dataSim_fitted.png"), 
       width=10, height=6)
```

```{r}
sim_finished <- dir(sim_dir, glue("mu_fitted_metrics_weekly.csv"), recursive=T) |> 
  str_sub(1, 6) |> unique()
metric_wk_vPrior <- inner_join(
  map_dfr(
    sim_finished,
    ~read_csv(glue("{sim_dir}/{.x}/mu_fitted_metrics_weekly.csv"), show_col_types=F) |>
      mutate(sim=paste0("dat_", str_sub(.x, -2, -1)))) |>
    group_by(sim) |>
  ...
  map_dfr(
    sim_finished,
    ~read_csv(glue("{sim_dir}/{.x}/mu_fitted_metrics_weekly_PRIORS.csv"), show_col_types=F) |>
      mutate(sim=paste0("dat_", str_sub(.x, -2, -1)))) |>
    group_by(sim) |>
    summarise(across(-1, mean)) |>
    ungroup() |>
