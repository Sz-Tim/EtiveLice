...
transformed data {
  array[2] int nStagesSex = {nStages, nStages};
...
parameters {
  matrix[nSurvCov, nStages] surv_beta_z; // p(surv) [Int, sal][Ch, Pr, Ad]
  matrix[2, nStages-1] pMoltF_beta_z; // [Int, temp][Ch-Pr, Pr-Ad]
  matrix[2, nStages-1] pMoltM_beta_z; // [Int, temp][Ch-Pr, Pr-Ad]
...
transformed parameters {
  ...
  matrix[2, nStages-1] pMoltM_beta; // [Int, temp][Ch-Pr, Pr-Ad]
  ...
  for(sex in 1:2) {
    for(farm in 1:nFarms) {
      for(day in 1:nDays) {
        ...
        trans_mx[sex, farm, day, nStages, nStages] = stage_Surv[farm, day, nStages];
      }
    }
  }
}
